...
class UserAccountServiceTest {
    ...
    @DisplayName("존재하지 않는 회원 ID를 검색하면, 비어있는 Optional을 반환환다.")
    @Test
    void givenNonexistentingUserId_whenSearching_thenReturnsOptionalUserData() {
        // given
    ...
    @DisplayName("회원 정보를 입력하면, 새로운 정보를 저장하여 가입시키고 해당 회원 데이터를 입력한다.")
    @Test
    void givenUserParams_whenSaving_thenSavesUserAccount() {
        // given
        UserAccount userAccount = createUserAccount("23Yong");
        UserAccount savedUserAccount = createSigningUpUserAccount("23Yong");
